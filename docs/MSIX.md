...
powershell -ExecutionPolicy Bypass -File publish-msix.ps1 `
  -PackageName 'COPY_FROM_PARTNER_CENTER' `
  -Publisher 'CN=COPY_FROM_PARTNER_CENTER' `
  -PublisherDisplayName 'COPY_FROM_PARTNER_CENTER' `
...
The staged application passed `--self-test`. The Store identity is still needed for the final package.
